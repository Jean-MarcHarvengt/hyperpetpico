...
static int processArray(unsigned char * array, int size, char * outfile, char * arrname) 
{
  FILE *fp_wr = stdout;
...
#define OUT_SIZE 65536+256
static unsigned char outBuffer[OUT_SIZE];

int main(int argc, char *argv[]) {
	FILE *fp_rd = fopen("bitmap.cru", "rb");
	fseek(fp_rd, 0L, SEEK_END);
	int size = ftell(fp_rd);
	fseek(fp_rd, 0L, SEEK_SET);
	fread(&outBuffer[sizeof(outBuffer)-size], size, 1, fp_rd);
	fclose(fp_rd);

	processArray(&outBuffer[sizeof(outBuffer)-size], size, "bitmap.h", "bitmap_bmp");

	int outPointer = UnPack(0, &outBuffer[sizeof(outBuffer)-size], &outBuffer[0], OUT_SIZE);

	FILE *fp_wr = fopen("decompressed.bin", "wb"); 
    {
  		fwrite(outBuffer, outPointer, 1, fp_wr);
      	fclose(fp_wr);
  		fprintf(stderr,
